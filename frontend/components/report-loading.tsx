...
export function ReportLoading({ sessionId }: ReportLoadingProps) {
  ...
  const handleRetry = async () => {
    setIsRetrying(true)
    try {
      const response = await fetch(`${API_BASE}/retry/${sessionId}`, {
        method: "POST",
      })

      if (!response.ok) {
        const errorData = await response.json().catch(() => null)
        const errorMessage = errorData?.detail || errorData?.error || "Failed to retry report generation"
      ...
      toast({
        title: "Retry started",
        description: "Report generation restarted. Please wait while we process your request.",
      })
    } catch (error) {
      console.error("Error retrying report generation:", error)
      const description = error instanceof Error ? error.message : "Retry failed. Please try again."
      toast({
        title: "Retry failed",
        description,
        variant: "destructive",
